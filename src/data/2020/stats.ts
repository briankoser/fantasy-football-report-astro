...
function getGameSlim(team, week, game, projectedPoints, winProjection, didWin) {
    ...
    let mvpPoints : number = Math.max.apply(this, starters.map(s => s.playerPoolEntry.appliedStatTotal));
    ...
    let lvpPoints : number = Math.min.apply(this, starters.map(s => s.playerPoolEntry.appliedStatTotal));
    ...
    let benchMvpPoints : number = Math.max.apply(this, bench.map(s => s.playerPoolEntry.appliedStatTotal));
    ...
    let positionPoints = Object.keys(espn.lineupSlots).map(position => { 
        ...
        let points : number = players.reduce( (acc, cur) => acc += cur.playerPoolEntry.appliedStatTotal, 0);
